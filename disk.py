...
# Ignora o aviso de depreciação do tipo 'u'
...
class Disk:
    ...
    Este disco é organizado em 32 blocos[cite: 5].
    Cada bloco tem 16 bits para dados (char) e 16 bits para ponteiro (short int)[cite: 4].
    """
    DISK_SIZE = 32
    NULL_POINTER = -1
    ...
    def __init__(self):
        """
        Inicializa os arrays que representam o disco.
        """
        # Array para os dados (1 caractere por bloco)
        try:
            # 'u' -> Py_UNICODE (representa o 'char' de 16 bits) [cite: 4]
        ...
        # 'h' -> signed short int (representa o 'short int' de 16 bits) [cite: 4]
        self.pointers = array.array('h', [0] * self.DISK_SIZE)
        
    def initialize_free_list(self):
        """
        Configura a lista encadeada inicial de blocos livres[cite: 32].
