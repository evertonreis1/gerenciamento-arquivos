...
def print_step(title):
    """Imprime um painel grande para destacar uma etapa da demonstração."""
...
def print_read_file(name, content):
    """Imprime o resultado de uma leitura de arquivo."""
    console.print(f"📖 [bold]Leitura de '[yellow]{name}[/yellow]':[/bold] \"{content}\"")

def print_disk_status(fs):
    """
    Imprime o estado atual de todos os blocos do disco em uma tabela bonita[cite: 21].
    """
    table = Table(title="💿 Status Atual do Disco", title_style="bold magenta", padding=0)
    ...
    for i in range(fs.disk.DISK_SIZE):
        ...
        # Formata o caractere de dado
        data_repr = f"'{data}'" if data != '\0' else "[dim]''[/dim]"
        
        # Formata o ponteiro
        pointer_repr = f"[red]NULL[/red]" if pointer == fs.disk.NULL_POINTER else str(pointer)
        
        # Adiciona cor se o bloco estiver alocado (dado não nulo)
...
def print_directory(fs):
    """
    Imprime a tabela de diretório[cite: 21].
    """
...
def print_free_list(fs):
    """
    Imprime os índices dos blocos vazios e o total[cite: 21].
    """
    ...
    # Percorre a lista encadeada de blocos livres
    nodes = []
